    ...
    <div class="flex">
        <a href="?admin=FormVehicles">
            <button class="bg-[#ddfae8] text-[#0D6344] border border-[#0D6344] rounded-md p-1 hover:bg-[#0D6344] hover:text-white flex items-center gap-1">
                ...
                <?= htmlspecialchars(t('vehicle_table_add_button'), ) ?>
            </button>
        </a>
    </div>
    ...
    <div class="overflow-x-auto mt-2 border border-gray-300 rounded-md">
        <table class=" min-w-full divide-y divide-gray-200 bg-white shadow-sm  overflow-hidden ">
            <thead class="bg-[#0D6344] text-white text-left text-xs font-semibold uppercase">
                <tr>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('vehicle_table_header_model'), ) ?></th>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('vehicle_table_header_license_plate'), ) ?></th>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('vehicle_table_header_type'), ) ?></th>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('vehicle_table_header_total_km'), ) ?></th>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('vehicle_table_header_status'), ) ?></th>
                    <th class="px-4 py-2" colspan="4"><?= htmlspecialchars(t('vehicle_table_header_actions'), ) ?></th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-10 border border-gray-300">
                <?php if (empty($vehicles)): ?>
                <tr class="odd:bg-white even:bg-gray-50">
                    <td class="px-4 py-6 text-sm text-gray-500 text-center" colspan="6"><?= htmlspecialchars(t('vehicle_table_no_data'), ) ?></td>
                </tr>
                <?php else: ?>
                ...
                <tr class="hover:bg-gray-50 ">
                    ...
                    <td>
                        ...
                        <button class="px-2 py-3" onclick="return confirm('<?= htmlspecialchars(t('confirm_delete_vehicle'), ) ?>');">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="size-4">
