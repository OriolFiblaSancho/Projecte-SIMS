...
<div class="max-w-3xl mx-auto mt-2">
    <div class="bg-white border border-gray-100 rounded-lg shadow-sm p-6">
        <div class="flex items-start justify-between gap-4 mb-6">
            <a href="/main?admin=ViewVehicles" class="mt-1 inline-block text-sm text-primary hover:underline">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
                    <path fill-rule="evenodd" d="M11.03 3.97a.75.75 0 0 1 0 1.06l-6.22 6.22H21a.75.75 0 0 1 0 1.5H4.81l6.22 6.22a.75.75 0 1 1-1.06 1.06l-7.5-7.5a.75.75 0 0 1 0-1.06l7.5-7.5a.75.75 0 0 1 1.06 0Z" clip-rule="evenodd" />
                </svg>
            </a>
            <div class="flex flex-wrap gap-3">
                <a href="/vehicles/delete/<?php echo urlencode($vehicle['vehicle_id']); ?>" onclick="return confirm('Are you sure you want to delete this vehicle?');" class="inline-flex items-center gap-2 bg-accent hover:brightness-95 text-white px-4 py-2 rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent">Delete</a>
                <a href="/main?admin=FormVehicles&edit=<?php echo urlencode($vehicle['vehicle_id']); ?>" class="inline-flex items-center gap-2 bg-primary hover:brightness-95 text-white px-4 py-2 rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">Edit</a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            function renderField($label, $value, $colSpan = 1) {
                ...
                ?>
                <div class="bg-[#CAF0D8] border border-gray-100 p-4 rounded-md <?php echo $col; ?>">
                    <p class="text-xs text-neutral-800 uppercase tracking-wider"><?php echo $label; ?></p>
                    <p class="mt-1 text-lg font-semibold text-gray-800"><?php echo $value; ?></p>
                </div>
                <?php
            }
