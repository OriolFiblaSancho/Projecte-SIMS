<div class="flex flex-col rounded-md p-3 max-h-[60vh] overflow-auto">
    <?php $paginationBase = '/main?admin=ViewGeofencing'; include_once __DIR__ . '/../../../../common/components/filterGeofencing.php'; ?>
    ...
    <div class="flex">
        <a href="?admin=FormGeofencing">
            <button class="bg-[#ddfae8] text-[#0D6344] border border-[#0D6344] rounded-md p-1 hover:bg-[#0D6344] hover:text-white flex items-center gap-1">
                ...
                <?= htmlspecialchars(t('geofence_table_add_button'), ENT_QUOTES) ?>
            </button>
        </a>
    </div>
    ...
    <div class="overflow-x-auto mt-2 border border-gray-300 rounded-md">
        <table class=" min-w-full divide-y divide-gray-200 bg-white shadow-sm  overflow-hidden ">
            <thead class="bg-[#0D6344] text-white text-left text-xs font-semibold uppercase">
                <tr>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('geofence_table_header_name'), ENT_QUOTES) ?></th>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('geofence_table_header_type'), ENT_QUOTES) ?></th>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('geofence_table_header_radius'), ENT_QUOTES) ?></th>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('geofence_table_header_coordinates'), ENT_QUOTES) ?></th>
                    <th class="px-4 py-2"><?= htmlspecialchars(t('geofence_table_header_actions'), ENT_QUOTES) ?></th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-10 border border-gray-300">
                <?php if (empty($zones)): ?>
                <tr>
                    <td class="px-4 py-6 text-sm text-gray-500 text-center" colspan="5"><?= htmlspecialchars(t('geofence_table_no_data'), ENT_QUOTES) ?></td>
                </tr>
                <?php else: ?>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-sm text-gray-700">
                        <!-- View Button -->
                        <a href="/main?admin=ViewGeofencingSingle&id=<?= $zone['zone_id'] ?>">
                        ...
                        <!-- Edit Button -->
                        <a href="/main?admin=FormGeofencing&edit=<?= $zone['zone_id'] ?>">
                        <button class="px-2 py-3">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="size-4">
                        ...
                        <!-- Delete Button -->
                        <a href="/geofencing/delete/<?= $zone['zone_id'] ?>">
                        <button class="px-2 py-3" onclick="return confirm('<?= htmlspecialchars(t('confirm_delete_zone'), ENT_QUOTES) ?>');">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="size-4">
