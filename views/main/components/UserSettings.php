...
<div id="userSettingsPanel" class="hidden fixed top-20 right-4 z-40 bg-white rounded-md shadow-lg border border-[#d6c3f0] w-[420px] max-w-[95vw] p-4">
   <div class="flex items-center justify-between mb-2">
    <h3 class="text-lg font-semibold text-[#2d0a4a]">Configuració d'usuari</h3>
    <button id="closeUserSettings" class="p-1 hover:opacity-80" aria-label="Tancar">
  ...
  <form id="userSettingsForm" method="post" action="/settings/save">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        <label class="block text-sm font-medium text-[#2d0a4a]">Nom</label>
        <input name="name" value="<?php echo htmlspecialchars($currentUser['name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
               class="mt-1 w-full rounded border px-3 py-2" placeholder="Opcional" />
      </div>
      <div>
        <label class="block text-sm font-medium text-[#2d0a4a]">Cognoms</label>
        <input name="last_name" value="<?php echo htmlspecialchars($currentUser['last_name'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
               class="mt-1 w-full rounded border px-3 py-2" placeholder="Opcional" />
      </div>
      <div>
        <label class="block text-sm font-medium text-[#2d0a4a]">Telèfon</label>
        <input name="phone" value="<?php echo htmlspecialchars($currentUser['phone'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
               class="mt-1 w-full rounded border px-3 py-2" placeholder="Opcional" />
      </div>
      <div>
        <label class="block text-sm font-medium text-[#2d0a4a]">Permís de conduir</label>
        <input name="driver_license" value="<?php echo htmlspecialchars($currentUser['driver_license'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
               class="mt-1 w-full rounded border px-3 py-2" placeholder="Opcional" />
      </div>
      <div>
        <label class="block text-sm font-medium text-[#2d0a4a]">Nom d'usuari</label>
        <input name="username" value="<?php echo htmlspecialchars($currentUser['username'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
               class="mt-1 w-full rounded border px-3 py-2" placeholder="Opcional" />
      </div>
      <div>
        <label class="block text-sm font-medium text-[#2d0a4a]">Email (només lectura)</label>
    ...
    <div class="flex justify-end gap-2 mt-3 md:col-span-2">
      <button type="button" id="cancelUserSettings" class="px-3 py-1 rounded border border-[#7E3FBC] text-[#2d0a4a] hover:bg-[#f2e9fb]">Cancel·lar</button>
      <button type="submit" class="px-3 py-1 rounded bg-[#7E3FBC] text-white hover:bg-[#6c36aa]">Desar</button>
    </div>
  </form>
</div>
      ...
      const json = await res.json();
        ...
        if (msg) {
          ...
          if (json.success) {
            ...
            msg.textContent = json.message || 'Desat correctament';
          } else {
